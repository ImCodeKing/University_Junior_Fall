...
typedef RT_MODEL_lzy_lyl_1221_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * lzy_lyl_1221(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(lzy_lyl_1221_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(lzy_lyl_1221_DW.HILReadEncoderTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://lzy_lyl_1221:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: lzy_lyl_1221 -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      fflush(stdout);
      result = hil_task_start(lzy_lyl_1221_DW.HILReadEncoderTimebase_Task,
        (t_clock) lzy_lyl_1221_P.HILReadEncoderTimebase_Clock, 500.0, -1)
        ;
      if (result == 0) {
